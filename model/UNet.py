...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.encoder[0])):
            ...
            # print(f'x1_encoded shape: {x1_encoded.shape}')
            x2_encoded = self.encoder[1][i](x2_encoded)
            # print(f'x2_encoded shape: {x2_encoded.shape}')
            x3_encoded = self.encoder[2][i](x3_encoded)
            # print(f'x3_encoded shape: {x3_encoded.shape}')
            
            if i < len(self.encoder[0]) - 1:  # Store skip connections where newest is first
                skips1.insert(0, x1_encoded)
                # print(f'skips1 shape: {skips1[0].shape}')
                skips2.insert(0, x2_encoded)
                # print(f'skips2 shape: {skips2[0].shape}')
                skips3.insert(0, x3_encoded)
                # print(f'skips3 shape: {skips3[0].shape}')
        ...
        # print(f'skiped_combined0 shape: {skiped_combined0.shape}')
        skiped_combined1 = self.combiner_128(skips1[1] + skips2[1] + skips3[1] )
        # print(f'skiped_combined1 shape: {skiped_combined1.shape}')
        skiped_combined2 = self.combiner_64(skips1[2] + skips2[2] + skips3[2] )
        # print(f'skiped_combined2 shape: {skiped_combined2.shape}')
        skiped_combined3 = self.combiner_32(skips1[3] + skips2[3] + skips3[3] )
        # print(f'skiped_combined3 shape: {skiped_combined3.shape}')
        ...
        # print(f'outc shape: {self.outc(x).shape}')
        return self.outc(x)
