...
def denoise(model, input_dir, output_dir, resolution=512, num_prev_frames=2, fine_layers=False, bottleneck_size=16, model_type='cnn', with_skip_connections=False, with_residual_connections=False, with_resnet_blocks=False):
    ...
    # get unique prefixes
    print(f"Denoising {len(all_files)} files")

    # Process frames sequentially
    all_outputs = []
    with torch.no_grad():
        # get first frame to get image dimensions
        frame1_img = Image.open(all_files[0]).convert('L')
        frame1_img_name = os.path.basename(all_files[0])
        # save the first frame and the second frame
        frame1_img.save(os.path.join(output_dir, f'{frame1_img_name}'))#, quality=95)
        frame2_img = Image.open(all_files[1]).convert('L')
        frame2_img_name = os.path.basename(all_files[1])
        frame2_img.save(os.path.join(output_dir, f'{frame2_img_name}'))#, quality=95)
        ...
        # save the last frame and the second-to-last frame
        second_to_last_frame = Image.open(all_files[-2]).convert('L')
        second_to_last_frame_name = os.path.basename(all_files[-2])
        second_to_last_frame.save(os.path.join(output_dir, f'{second_to_last_frame_name}'))#, quality=95)
        last_frame = Image.open(all_files[-1]).convert('L')
        last_frame_name = os.path.basename(all_files[-1])
        last_frame.save(os.path.join(output_dir, f'{last_frame_name}'))#, quality=95)
...
if __name__ == "__main__":
    ...
    num_frames = 0
    for clip in clips:
        num_frames += len(glob.glob(os.path.join(args.output_dir, clip, '*.tif')))
    print(f"Number of frames saved: {num_frames}")
    num_frames_expected = 0
    for clip in clips:
        num_frames_expected += len(glob.glob(os.path.join(args.input_dir, clip, '*.tif'))) - 2 * args.num_prev_frames
