...
FRAMES_DIR = f"/root/covid19_ultrasound/data/pocus_videos_frames/convex_baseline_all_flat_symlinks"
VIDEO_DIR = f"/root/covid19_ultrasound/data/pocus_videos_frames/convex_baseline"
...
results_path = f"/root/covid19_ultrasound/data/pocus_videos_frames_n2v" 
model_path = f'{RESULTS_DIR}/n2v_POTUS_results/best_model-v2.ckpt'
...
for video in tqdm(os.listdir(videos_to_denoise), desc="Processing videos"):
    ...
    os.makedirs(os.path.join(results_path, video), exist_ok=True)
    ...
    for i, frame in enumerate(denoised_frames):
        frame.save(os.path.join(results_path, video, f"frame_{i:04d}.png"))