...
def format_frame_path(base_path, frame_num):
    return os.path.join(base_path, f"{frame_num:06d}.png")


class NoisyVideoDatasetWithWindowSizeCVA(Dataset):
    ...
    def load_image_files(self):
        ...
        for f in image_files:
            ...
            if match:
                ...
                required_files = [
                    format_frame_path(base_path, frame_number - i) for i in range(1, self.window_size//2+1)
                ] + [
                    format_frame_path(base_path, frame_number + i) for i in range(1, self.window_size//2+1)
                ]
    ...
    def get_future_frame_path(self, img_path, offset=1):
        frame_num = int(re.findall(r'(\d+)\.png', img_path)[0])
        future_frame_num = frame_num + offset
        future_img_path = img_path.replace(f'{frame_num:06d}.png', f'{future_frame_num:06d}.png')
        return future_img_path

    def get_previous_frame_path(self, img_path):
        frame_num = int(re.findall(r'(\d+)\.png', img_path)[0])
        prev_frame_num = frame_num - 1
        prev_img_path = img_path.replace(f'{frame_num:06d}.png', f'{prev_frame_num:06d}.png')
