...
def format_frame_path(base_path, prefix, frame_num):
    return os.path.join(base_path, f"{prefix}{frame_num:04d}.png")



class NoisyVideoDatasetWithWindowSizePOCUS(Dataset):
    def __init__(self, base_folder, transform=None, target="l1", window_size=31):
    ...
    def load_image_files(self):
        ...
        if len(image_files) == 0:
            print("No png files found! Check path and file extensions.")
            return []
        ...
        for f in image_files:
            ...
            if match:
                prefix = match.group(1)  # gets 'some_string_frame_'
                frame_number = int(match.group(2))                
                base_path = os.path.dirname(f)

                # Check if previous and next frames exist
                required_files = [
                    format_frame_path(base_path, prefix, frame_number - i) for i in range(1, self.window_size//2+1)
                ] + [
                    format_frame_path(base_path, prefix, frame_number + i) for i in range(1, self.window_size//2+1)
                ]
    ...
    def get_future_frame_path(self, img_path, offset=1):
        ...
        if match:
            ...
            ext = match.group(3)     # gets '.png'
            future_frame_num = frame_num + offset
            future_frame_path = f"{prefix}{future_frame_num:04d}{ext}"
            return future_frame_path
        else:
            raise ValueError(f"Could not find frame number in {img_path}")
    ...
    def get_previous_frame_path(self, img_path):
        ...
        if match:
            ...
            prev_img_path = f"{prefix}{prev_frame_num:04d}{ext}"
